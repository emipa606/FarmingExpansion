...
namespace FarmingExpansion
{
    ...
    public class UseChemicalJobDriver : JobDriver {
		...
		private float applyChemicalWorkDone = 0f;

		// The targeted plant
		private Plant Plant {
			get => (Plant)job.GetTarget(TargetPlant).Thing;
		}
		...
		public override bool TryMakePreToilReservations(bool errorOnFailed) {
			...
			if (pawn.Reserve(job.targetA, job, 1, -1, null) && pawn.Reserve(job.targetB, job, 1, -1, null)) {
				return true;
			}
			return false;
		}
		...
		protected override IEnumerable<Toil> MakeNewToils() {
			...
			Toil pesticideReserved = Toils_Reserve.Reserve(TargetChemical, 1, -1, null);
			yield return pesticideReserved;

			// Fail conditions - prevents having to write long Lambdas
			yield return ToilFailConditions.FailOnSomeonePhysicallyInteracting(ToilFailConditions.FailOnDespawnedNullOrForbidden(Toils_Goto.GotoThing(TargetChemical, PathEndMode.ClosestTouch), TargetChemical), TargetChemical);
			...
			yield return ToilFailConditions.FailOnDestroyedNullOrForbidden(Toils_Haul.StartCarryThing(TargetChemical, false, true, false), TargetChemical);
			yield return Toils_Reserve.Release(TargetChemical);

			// Gather other pesticide items if neccessary
			yield return Toils_Haul.CheckForGetOpportunityDuplicate(pesticideReserved, TargetChemical, TargetIndex.None, true, null);

			// Go to the targeted plant
			Toil goToPlantCell = Toils_Goto.GotoCell(TargetPlant, PathEndMode.Touch);
			goToPlantCell.AddFinishAction(delegate {
				job.count = job.GetTarget(TargetChemical).Thing.stackCount;
			});
            ...
            Toil toil = new Toil
            {
                ...
                tickAction = delegate
                {
                    ...
                    pawn.skills.Learn(SkillDefOf.Plants, 0.02f, false);

                    applyChemicalWorkDone += 6 * pawn.GetStatValue(StatDefOf.PlantWorkSpeed, true);
				...
				if (applyChemicalWorkDone >= Plant.def.plant.sowWork) {
                    ...
                    switch (chemical)
                    {
                        ...
                        case "FE_DDT":
                            ...
                            if (ChemicalAddedCheck.FungicideAppliedOnto.Contains(Plant)) ChemicalAddedCheck.FungicideAppliedOnto.Remove(Plant);
                            if (ChemicalAddedCheck.HerbicideAppliedOnto.Contains(Plant)) ChemicalAddedCheck.HerbicideAppliedOnto.Remove(Plant);
                            if (ChemicalAddedCheck.PesticideAppliedOnto.Contains(Plant)) ChemicalAddedCheck.PesticideAppliedOnto.Remove(Plant);
                            break;
			...
			toil.WithProgressBar(TargetIndex.A, () => applyChemicalWorkDone / (Plant.def.plant.sowWork), true, -0.5f);
			...
			yield return Toils_Reserve.Reserve(TargetPlant, 1, -1, null);
		...
		/// <summary>
		/// Finds a plant, which is preferably as close to the pawn as possible but at least within the same zone, and sets it as the new job target. This results in the pawn going plant to plant
		/// as it applies chemicals without having to start the job over and collect a new chemical item to replace the used one.
		/// </summary>
		private Toil ApplyOntoRandomNearbyPlantNext() {
            ...
            Toil toil = new Toil
            {
                ...
                initAction = delegate
                {
                    if (pawn.carryTracker.CarriedThing == null || pawn.carryTracker.CarriedThing.def != job.targetB.Thing.def)
                    {
                    ...
                    else
                    {
                        ...
                        bool predicate(IntVec3 cellBeingChecked)
                        {
                            ...
                            if ((cellBeingChecked.GetZone(pawn.Map) as Zone_Growing) == null)
                            {
                                return false;
                            }
                            if (job.GetTarget(TargetPlant).Cell.GetZone(pawn.Map) != (cellBeingChecked.GetZone(pawn.Map) as Zone_Growing))
                            {
                                return false;
                            }
                            if (!UseChemicalWorkGiver.IsGrowingZonesWithChemicalList.Contains(cellBeingChecked.GetZone(pawn.Map) as Zone_Growing))
                            {
                                return false;
                            }
                            if (cellBeingChecked.GetPlant(pawn.Map).def != (job.GetTarget(TargetPlant).Cell.GetZone(pawn.Map) as Zone_Growing).GetPlantDefToGrow())
                            {
                                return false;
                            }
                            if (!ReservationUtility.CanReserveAndReach(pawn, cellBeingChecked.GetPlant(pawn.Map), PathEndMode.ClosestTouch, DangerUtility.NormalMaxDanger(pawn)))
                            {
                                return false;
                            }
                        ...
                        if (CellFinder.TryFindRandomCellNear(pawn.Position, pawn.Map, 1, predicate, out IntVec3 newPlantCellLocation) || CellFinder.TryFindRandomCellNear(pawn.Position, pawn.Map, 2, predicate, out newPlantCellLocation))
                        {
                        ...
                        {
                            foreach (IntVec3 cell in job.GetTarget(TargetPlant).Cell.GetZone(pawn.Map).Cells)
                            {
