...
namespace FarmingExpansion
{
    ...
    /// <summary>
    /// Checks every minute (3600 ticks) if all of the chemical-applied plants still exists. If not, removes the plant's entry in the Dictionary
    /// containing all chemical-applied plants so that chemicals can be applied onto a new plant on that cell on that map
    /// </summary>
    class ChemicalAddedCheck : GameComponent {
		...
		/// <summary>
		/// Empty constructor with no function other than to prevent an error from when the vanilla game tries to access the constructor
		/// </summary>
		public ChemicalAddedCheck(Game game) : base() {
		...
		/// <summary>
		/// Checks whether all plants that chemicals have been applied onto still exists. If not, removes that plant's entry in the dictionary
		/// </summary>
		public override void GameComponentTick() {
			...
			if (FunctionWasLastRunOnTick + 3600 <= Current.Game.tickManager.TicksGame) {
				...
				Thread t = new Thread(() => {
					foreach (List<Plant> list in new List<List<Plant>> { FungicideAppliedOnto, HerbicideAppliedOnto, PesticideAppliedOnto, DDTAppliedOnto, ChemicalSprayAppliedOnto })
                    {
		...
		public override void ExposeData() {
			...
			Scribe_Collections.Look(ref UseChemicalWorkGiver.IsGrowingZonesWithChemicalList, "IsGrowingZonesWithChemicalList", LookMode.Reference);
		}
	}
}
